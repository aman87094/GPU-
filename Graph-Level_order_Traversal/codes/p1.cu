...
__global__void kernal(int* offset, int* csr, bool* processed, int* lvlarr,int V){
  ...
  if(tid%32=0){
    ...
    int en=offset[tid+1];
    if(tid<V){
      while(processed[tid]==False){
          for(int i=st;i<en;i++){
            lvlarr[i]+=lvlarr[tid]+1;
          }
      }
      printf("%d %d",tid,lvlar[tid]);
    }
  }
}
...
int main (int argc, char **argv) {
  ...
  cudaMalloc(&d_offset,sizeof(int)*(V+1));
  cudaMalloc(&d_csr,sizeof(int)*E);
  
  cudaMemcpy(d_offset,hOffset,sizeof(int)*(V+1),cudaMemcpyHostToDevice);
  cudaMemcpy(d_csr,hCsr,sizeof(int)*E,cudaMemcpyHostToDevice);

  bool *processed;
  cudaMalloc(&processed, sizeof(bool)*(V+1));
  cudaMemset(processed, false, sizeof(bool)*(V+1));
  bool trueValue = true;

  int *lvlarr;
  cudaMalloc(&lvlarr,sizeof(int)*(V+1));
  cudaMemset(lvlarr,0,sizeof(int));
  cudaMemcpy(processed, &trueValue, sizeof(bool), cudaMemcpyHostToDevice);

  kernal<<<bolck*32,32>>>(d_offset,d_csr,processed,lvlarr,V);
